...
export const usePromptStore = defineStore('prompt', () => {
  ...
  // ⚡ Bolt Optimization: Static output order moved outside computed to prevent reallocation on every keystroke
  ...
  const generatedPrompt = computed(() => {
    ...
    OUTPUT_ORDER.forEach((field) => {
      ...
      if (value) {
        // ⚡ Bolt Optimization: Trim only once per field
    ...
    // 特化情報の出力
    const specializationValue = formData.value.specialization
    if (specializationValue) {
      const trimmedSpecialization = specializationValue.trim()
  ...
  // ⚡ Bolt Optimization: Extracted static presets out of the function to prevent
  // large object reallocation on every function call.
...
\`\`\``
  ...
  // ⚡ Bolt Optimization: Extract commonSamples and categorySamples out of generateSampleData
  // to avoid recreating these large objects on every function call.
  // 共通フィールドのサンプル
  const COMMON_SAMPLES: Partial<PromptFormData> = {
    role: '経験豊富な専門家として',
  ...
  const generateSampleData = () => {
    ...
    Object.entries(COMMON_SAMPLES).forEach(([key, value]) => {
      if (visibleFields.value.includes(key)) {
        updateField(key as keyof PromptFormData, value as string)
      }
    })
